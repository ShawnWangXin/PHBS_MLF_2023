...
class Dataset:
    def __init__(self, start, end, model, matrix_dir,flag):
        ...
        if self.model == 'GAT' or self.model == 'GCN':
    ...
    def __getitem__(self, idx):   
        ...
        data_file = f'./data/MLP/{self.flag}/{self.start}_{self.end}/{date}.h5'
        if os.path.exists(data_file):
            with h5py.File(data_file, 'r') as hf:
                Data, label, mktcp_data, index_bytes = hf['Data'][:], hf['label'][:],  hf['mktcp_data'][:], hf['index'][:]    
            ...
            os.makedirs(f'./data/MLP/{self.flag}/{self.start}_{self.end}', exist_ok=True)
        ...
        return Data, label, None, mktcp_data, index, date # 返回一天的feature数据，和一天的标签
